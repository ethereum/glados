...
use ethportal_api::{HistoryContentKey, HistoryContentValue, OverlayContentKey};
use thiserror::Error;
use tracing::warn;
...
pub async fn content_is_valid(
    ...
    match check_content_correctness(content_key, content, config).await {
        Ok(_) => Ok(true),
...
async fn check_content_correctness(
    ...
    match content {
        ...
        HistoryContentValue::BlockBody(b) => {
            ...
            let header = fetch_block_header(&block_hash, config).await?;
