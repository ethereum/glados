function radius_stacked_chart(data) {
  ...
  data.forEach(function (node, idx, arr) {
    const nodePrefix = Number(
      BigInt(node.node_id) >> (64n - BigInt(bucket_bit_width)),
    );
  ...
  part.forEach(
    (covers, prefix, arr) =>
      (compiledData[num_buckets + prefix] = {
        prefix: prefix,
        coverage: "part",
        fraction: covers,
      }),
  );
  const indexedData = d3.index(
    compiledData,
    (d) => d.prefix,
    (d) => d.coverage,
  );
  ...
  // Add X axis
  const x = d3.scaleLinear().domain([0, num_buckets]).range([0, width]);
  const ticks = Array(17)
    .fill(0)
    .map((none, index) => index * 16);
  ...
  function hoverAppear(event, d) {
    ...
    hover.html(
      `Data Prefix: 0x${buckethex}<br>Type: ${coverageName}<br>Coverage multiple: ${coverage}`,
    );
  ...
  function hoverFollow(event, d) {
    ...
    hover.style("left", hoverX + "px").style("top", hoverY + "px");
  }
...
function radius_node_id_scatter_chart(data) {
  ...
  function hoverFeature(event, d) {
    ...
    hover
      .html(
        `Client Name: ${latestClientString}<br>Node ID: ${d.node_id_string}<br>Data Radius: ${d.data_radius}%`,
      )
