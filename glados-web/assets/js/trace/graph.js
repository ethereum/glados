...
function ForceGraph(
  ...
  if (sortByNodeId) {
    nodes = d3.map(nodes, (node, i) => ({
      id: N[i],
      fixedX: calculateNodeIdX(node.id, width) - width / 2,
      ...node,
    }));
  } else {
  ...
  const svg = d3
    ...
    .attr(
      "style",
      "max-width: 100%; height: auto; height: intrinsic; padding-left: 25px;",
    );
  ...
  function ticked() {
    if (sortByNodeId) {
      ...
      node.attr("cx", (d) => d.fixedX).attr("cy", (d) => d.y);
    } else {
      ...
      link
        ...
        .attr("y1", (d) =>
          enforceBorder(d.source.y, -height / 2, height / 2 - 24),
        )
        .attr("x2", (d) => enforceBorder(d.target.x, -width / 2, width / 2))
        .attr("y2", (d) =>
          enforceBorder(d.target.y, -height / 2, height / 2 - 24),
        );
  ...
  function force(alpha) {
    ...
    for (const node of nodes) {
      if (node.x < -width / 2 + padding)
        node.vx += (-width / 2 + padding - node.x) * adjustedStrength;
      if (node.x > width / 2 - padding)
        node.vx += (width / 2 - padding - node.x) * adjustedStrength;
      if (node.y < -height / 2 + padding)
        node.vy += (-height / 2 + padding - node.y) * adjustedStrength;
      if (node.y > height / 2 - padding)
        node.vy += (height / 2 - padding - node.y) * adjustedStrength;
    }
  }
